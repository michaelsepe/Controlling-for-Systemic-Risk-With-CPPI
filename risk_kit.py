...
def get_ffme_returns():
    ...
    """
    me_m = pd.read_csv("/Users/mikesepe/Downloads/Portfolios_Formed_on_ME_monthly_EW.csv",
...
def get_ind_file(filetype):
    ...
    if filetype is "returns":
        ...
        name = "size"
        divisor = 1
    ind = pd.read_csv(f"/Users/mikesepe/Downloads/ind30_m_{name}.csv", header=0, index_col=0)/divisor
...
def drawdown(returns_series: pd.Series):
    ...
    produces the wealth index as well as 
    the drawdowns
    
    this returns these values in a DF 
    in which any NaNs will be droppped
    '''
    
    wealthindex = 1000*(returns_series+1).cumprod()
...
def get_hfi_returns():
    '''
    load and format hedge fund index returns
    '''
    
    hfi = pd.read_csv('/Users/mikesepe/Downloads/history.csv',
                      header=0, index_col=0,sep=';', parse_dates=True)
    for col in range((hfi.shape[1])):
        hfi.iloc[:,col] = hfi.iloc[:,col].str.rstrip("%").astype(float)
    hfi = hfi/100
...
def skewness(r):
    '''
    alternative to scipy skew measure it computes
    the skewness of the series or DF and returns a float or series
    '''
    
    demeaned_r = r - r.mean()
    #using population std dev (dof=0)
...
def kurtosis(r):
    '''
    alternative to scipy kurtosis measure it computes
    the kurtosis of the series or DF and returns a float or series
    '''
...
def is_normal(r, level = 0.01):
    '''
    applies jb test at pval = 0.01
    '''
    statistic, pvalue = scipy.stats.jarque_bera(r)
    return pvalue > level

def semideviation(r):
    '''retunrs the semideviation of a DF
    '''
    is_neg = r < 0 
